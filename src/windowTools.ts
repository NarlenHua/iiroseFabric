...
/**
  ...
  * @returns 
  */
function createFabrcWindow(id: string, width: number, workSpace: HTMLElement = windowTools.createItem('div', `${id}-workspace`, fabricStyle.class["fabric-window-workspace"]), title: string = id, height: number = width / 4 * 3) {
  // 最外层窗口       
  ...
  // 窗口加入列表
  windowList.push(fabricWindow);
  maxZindex = maxZindex + 1;
  fabricWindow.style.zIndex = `${maxZindex}`;
  ...
  // 添加改变显示层级的功能
  fabricWindow.onmousedown = () => {
    // fabricWindow.onmousedown = (e) => {
    // console.log('显示层级', fabricWindow.style.zIndex, e.clientX, e.clientY);
    // 先把比它大的都减小一层
    for (let i = 0; i < windowList.length; i++) {
      if (parseInt(windowList[i].style.zIndex) > parseInt(fabricWindow.style.zIndex)) {
        windowList[i].style.zIndex = `${parseInt(windowList[i].style.zIndex) - 1}`
      }
    }
    // 再把自己设置成最高的
    fabricWindow.style.zIndex = `${maxZindex}`;
  ...
  // 适配移动端
  fabricWindow.ontouchstart = () => {
    // fabricWindow.ontouchstart = (e) => {
    // console.log('显示层级', fabricWindow.style.zIndex, e.touches[0].clientX, e.touches[0].clientY);
    // 先把比它大的都减小一层
    for (let i = 0; i < windowList.length; i++) {
      if (parseInt(windowList[i].style.zIndex) > parseInt(fabricWindow.style.zIndex)) {
        windowList[i].style.zIndex = `${parseInt(windowList[i].style.zIndex) - 1}`
 ...
 * @returns 菜单元素
 */
function createMenuItem(name: string, fabricWindow?: HTMLElement): HTMLElement {
  // 一级菜单
  ...
  if (fabricWindow != undefined && windowList.includes(fabricWindow)) {
    fabricMenuItem.onclick = () => {
      // fabricMenuItem.onclick = (e) => {
      // console.log('显示层级', fabricWindow.style.zIndex, e.clientX, e.clientY);
      // 先把比它大的都减小一层
      for (let i = 0; i < windowList.length; i++) {
        if (parseInt(windowList[i].style.zIndex) > parseInt(fabricWindow.style.zIndex)) {
      ...
      // 再把自己设置成最高的
      fabricWindow.style.zIndex = `${maxZindex}`;
      windowTools.turnDisplay(fabricWindow);
    };
  }
